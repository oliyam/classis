  ...
  draw_vessel(vessel, selected){
    ...
    if (selected && !ded)
      ...
      for(let i=0;i<vessel.pos.length;i++){
        ...
        if(!ded){
          this.draw_triangle(p, 15, 'lightgreen')
          this.ctx.strokeStyle = 'green'
          this.ctx.stroke()
      ...
      msg)
        ...
        this.loading = setInterval(()=>{
          ...
          this.ctx.font = px + "px monospace";       
           ...
           if (msg) {
            this.ctx.fillStyle = "red"
            this.ctx.clearRect(x, y + px * (line - 1 + 0==this.msg_line%4?this.msg_line%4-1:1), msg_l, 0==this.msg_line%4?px*4:px);
